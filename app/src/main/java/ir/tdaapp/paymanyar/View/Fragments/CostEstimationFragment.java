...
import ir.tdaapp.paymanyar.Presenter.P_CostEstimationFragment;
...
//صفحه متره و برآورد
public class CostEstimationFragment extends BaseFragment implements S_CostEstimationFragment, View.OnClickListener {

    public final static String TAG = "CostEstimationFragment";

    Toolbar toolbar;
    P_CostEstimationFragment p_costEstimationFragment;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.cost_estimation_fragment, container, false);

        findItem(view);
        implement();
        setToolbar();

        p_costEstimationFragment.start();
    ...
    void implement() {
        ...
        p_costEstimationFragment = new P_CostEstimationFragment(getContext(), this);
    ...
    void findItem(View view) {
        ...
        txt_Suggested = view.findViewById(R.id.txt_Suggested);
        txt_Above = view.findViewById(R.id.txt_Above);
            ...
            public void onClickFile(VM_FileUploadAnalizeTender file) {
                FileTypeOrderDialog fileTypeOrderDialog = new FileTypeOrderDialog(new onClickFileTypeOrderDialog() {
                    ...
                    public void clickedFile() {
                        p_costEstimationFragment.openFile(getActivity(), file);
                    }
                });
    ...
    public void onSelectedFile(VM_FileUploadAnalizeTender val, File file) {
        p_costEstimationFragment.checkValidationFile(file, val);
    }
    ...
    public void onValidFile(VM_FileUploadAnalizeTender val, File file) {
        ...
        val.setType(p_costEstimationFragment.getTypeFile(file));
    ...
    public VM_InputAnalizeTender onGetInputAnalize(List<String> fileUrls) {
        ...
        try {
            ...
            //در اینجا پیشنهادی ست می شود
            input.setSuggested(Float.valueOf(txt_Suggested.getText().toString()));

            //در اینجا بالاسری ست می شود
            input.setAbove(Float.valueOf(txt_Above.getText().toString()));

            //در اینجا منطقه ست می شود
            input.setZone(Float.valueOf(txt_zone.getText().toString()));

            //در اینجا تجهیزات کارگاه ست می شود
            input.setEquippingTheIngotWorkshop(Float.valueOf(txt_Equipping_the_ingot_workshop.getText().toString()));

            //در اینجا بهنگام ست می شود
            input.setTimely(Float.valueOf(txt_Timely.getText().toString()));
    ...
    public boolean isValidInputs() {
        ...
        if (!Validation.Required(txt_Above, getString(R.string.ThisValueMust_be_Filled))) {
            valid = false;
        }

        if (!Validation.Required(txt_zone, getString(R.string.ThisValueMust_be_Filled))) {
            valid = false;
        }
        ...
        try {
            ...
            if (bundle != null) {
                ...
                if (bundle.get("Id") != null) {
                    ...
                    p_costEstimationFragment.getDetailItem();
    ...
    public void onSetAnaliseInfo(VM_AnaliseInfo analiseInfo) {
        ...
        try {
            ...
            try {
                txt_Suggested.setText(analiseInfo.getSuggested() + "");
                txt_Above.setText(analiseInfo.getAbove() + "");
                txt_zone.setText(analiseInfo.getZone() + "");
                txt_Equipping_the_ingot_workshop.setText(analiseInfo.getEquippingTheIngotWorkshop() + "");
                txt_Timely.setText(analiseInfo.getTimely() + "");
            } catch (Exception e) {
            }
                ...
                for (int i = 0; i < files.size(); i++) {
                    try {
                        ...
                        if (u.length > 0) {
                            ...
                            FileUploadAnalizeTenderType type = p_costEstimationFragment.getTypeFile(format);
                            file.setPath(name);
                            file.setType(type);
    ...
    void setToolbar() {
        ...
        toolbar.setTitle(getContext().getResources().getString(R.string.CostEstimation2));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_ShowSteps:
                ...
                p_costEstimationFragment.addOrder();
                break;
            case R.id.btn_Home:
                    ...
                    public void onPermissionsChecked(MultiplePermissionsReport report) {
                        if (!fileName.equalsIgnoreCase("")) {
                            ...
                            if (file.exists()) {
                                ...
                                p_costEstimationFragment.downloadFile(t);
                            }
                        }
                    }
    ...
    public void onDestroy() {
        super.onDestroy();
        p_costEstimationFragment.cancel(TAG);
    }
    ...
    void clear() {
        new Thread(() -> {
            try {
                ...
                Thread.sleep(100);
                clear2("txt_Equipping_the_ingot_workshop");
    ...
    void clear2(String val) {
        handler_clear.post(() -> {
            switch (val) {
                case "txt_Timely":
                    txt_Timely.setError(null);
                    txt_Timely.setText("");
                    break;
                case "txt_Equipping_the_ingot_workshop":
                    txt_Equipping_the_ingot_workshop.setError(null);
                    txt_Equipping_the_ingot_workshop.setText("");
                    break;
                case "txt_zone":
                    txt_zone.setError(null);
                    txt_zone.setText("");
                    break;
                case "txt_Above":
                    txt_Above.setError(null);
                    txt_Above.setText("");
                    break;
                case "txt_Suggested":
                    txt_Suggested.setError(null);
