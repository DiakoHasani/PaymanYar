...
public class PowerSupplyNetworkAdapter extends RecyclerView.Adapter<PowerSupplyNetworkAdapter.MyViewHolder> {
  ...
  public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
    ...
    try {
      ...
      if (powerSupplyNetworks.get(position).isActive()){
        holder.upgradeButton.setVisibility(View.VISIBLE);
      }else {
        holder.upgradeButton.setVisibility(View.GONE);
      }
      holder.upgradeButton.setOnClickListener(view -> {
        clickPowerSupplyNetwork.upgrade(powerSupplyNetworks.get(position).getId());
      });
